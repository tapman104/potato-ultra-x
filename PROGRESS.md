...
- `app/src/main/java/com/potato/player/feature/player/controls/SubtitleAppearanceDialog.kt`: Nested modal dialog launched from `SubtitleTrackDialog` allowing real-time adjustment of subtitle size (`sub-scale` float) and position (`sub-pos` int, inverted `0f=bottom, 1f=top`) with apply/reset/cancel options.
- `app/src/main/java/com/potato/player/feature/player/controls/SubtitleTrackDialog.kt`: Compose modal dialog displaying available subtitle streams (`TrackInfo`), an "Off" option (`sid = -1`), an option to open `SubtitleAppearanceDialog`, and a document picker launcher (`ActivityResultContracts.OpenDocument()`) for loading external subtitle files.
...
- `app/src/main/java/com/potato/player/feature/player/PlayerViewModel.kt` (`onLoadExternalSubtitle` / `resolveSubtitlePath`)
  - *Status*: **FIXED**. Migrated to `MediaMetadataRepository.copySubtitleToCache` executing asynchronously on `Dispatchers.IO`, eliminating main-thread disk I/O and UI stutter.
...
- `app/src/main/java/com/potato/player/feature/player/PlayerScreen.kt` & `HomeScreen.kt` (`resolveFileName` / `resolveTitle`)
  - *Status*: **FIXED**. Migrated to `MediaMetadataRepository.resolveDisplayName` on `Dispatchers.IO` via `LaunchedEffect`, removing main-thread `ContentResolver.query()` calls.
- `app/src/main/java/com/potato/player/engine/MpvEngine.kt` & `MpvSurface.kt` (`prepareDecoderSwitch` & rotation VO teardown)
  - *Status*: **FIXED**. Implemented `prepareDecoderSwitch()` in `MpvEngine` to detach VO before changing codecs (`hwdec`), waiting for `video-reconfig` event. Added atomic `isRotating` flag across `MpvSurface`, `MainActivity.onConfigurationChanged`, and `AppNavigation` so rotations detach surface without destroying `vo=gpu`.
- `app/src/main/java/com/potato/player/engine/PlayerRepository.kt` (`flushPendingSeeks` & seek debouncing)
  - *Status*: **FIXED**. Added seek debouncing (`delay(80)` inside `repoScope`) and `isSeekingGate` across `seekCommit` and `seekExactRelative` to prevent slider drags and taps from overwhelming `PipelineWatcher` and causing audio/video stutter.
...
- `app/src/main/java/com/potato/player/feature/player/PlayerScreen.kt` & `PlayerRepository.kt` (`HoldToFastForward` / `startFastForward` / `stopFastForward`)
  - *Risk*: If the user initiates a long-press while already running at a custom speed (e.g., `1.5x`), `normalPlaybackSpeed` stores `1.5x`. However, if an `onPropertyChange(MpvProp.SPEED)` event arrives while `_isFastForwarding.value == true` or if `setPlaybackSpeed()` is invoked mid-hold, `normalPlaybackSpeed` can be overwritten, failing to restore the correct original speed on release.
- `app/src/main/java/com/potato/player/feature/player/PlayerScreen.kt` (`awaitEachGesture` / `isLongPressActive`)
  - *Risk*: If a long-press touch gesture is interrupted by system dialogs, incoming calls, or multi-touch pointer cancellation, `awaitPointerEvent()` exits its loop when pointers are cancelled, but `isLongPressActive = false` and `viewModel.stopFastForward()` might not execute reliably if the pointer event stream terminates via `PointerEventPass` cancellation without triggering `onRelease`.
- `app/src/main/java/com/potato/player/feature/home/HomeViewModel.kt` & `PlayerViewModel.kt` (`lockToPortrait` / `lockToLandscape`)
...
- `app/src/main/java/com/potato/player/MainActivity.kt` (`handleViewIntent`)
  - *Risk*: Taking persistable URI permissions via `takePersistableUriPermission` without verifying `Intent.FLAG_GRANT_PERSISTABLE_URI_PERMISSION` first can throw `SecurityException` (currently caught and swallowed), and checking readability via `openFileDescriptor(uri, "r")?.close()` performs main-thread disk I/O immediately before `navController.navigate()`.
- `app/src/main/java/com/potato/player/engine/MpvEngine.kt` (`destroy` & `init`)
  - *Status*: **FIXED (Phase 5/6 transition)**. Added `initialized.compareAndSet` guards (`AtomicBoolean`) with warning logs in both `init()` (`false -> true`) and `destroy()` (`true -> false`) to prevent double-init `SIGABRT` mutex crashes when launched via external `ACTION_VIEW` intent and ensure clean re-initialization after full teardown.
- `app/src/main/java/com/potato/player/MainActivity.kt` (`ACTION_VIEW` orientation & `onDestroy` reset) & `AppNavigation.kt` (`PlayerRoute` back navigation)
  - *Status*: **FIXED (Phase 5/6 transition)**. Fixed `HomeScreen` portrait ghost flash by detecting `intent?.action == Intent.ACTION_VIEW` in `onCreate()` and setting `requestedOrientation = ActivityInfo.SCREEN_ORIENTATION_SENSOR_LANDSCAPE` before `setContent`. Fixed rotation close glitch by setting `requestedOrientation = ActivityInfo.SCREEN_ORIENTATION_UNSPECIFIED` in `onDestroy()` when `isFinishing`, and setting `ActivityInfo.SCREEN_ORIENTATION_PORTRAIT` before calling `navController.popBackStack()` in `AppNavigation.kt`.
- `app/src/main/java/com/potato/player/feature/player/PlayerScreen.kt` & `PlayerViewModel.kt` (`showMoreMenu` vs `showSpeedDialog`)
  - *Risk*: When `PlayerRightSideSheet` is visible (`visible = uiState.showMoreMenu || uiState.showSpeedDialog`), closing the sheet via `onDismiss` invokes both `viewModel.onMoreMenuDismiss()` and `viewModel.onDismissSpeedDialog()`. When `showSpeedDialog` is true and the user taps the more button on `PlayerTopBar`, `showMoreMenu` is toggled while `showSpeedDialog` remains true, causing redundant and conflicting state updates.
