...
### Phase 5: Home Screen Refinement
- [ ] **Modern Media Hub & File Explorer (`HomeScreen.kt`)**
  * **Premium UI Aesthetics**: Glassmorphic dark cards, vibrant gradients, and smooth entry animations for media items.
  * **Storage Scanner & Permission Flow**: Request `READ_MEDIA_VIDEO` (Android 13+) / `READ_EXTERNAL_STORAGE` safely, scanning local device directories for playable video files (`.mp4`, `.mkv`, `.webm`, `.avi`, `.mov`).
  * **Recent Playback History**: Persist last-played videos and their playback positions (`timePos`) using `DataStore` or `Room` so users can resume instantly where they left off.
  * **System File Picker**: Retain a quick *"Open File"* floating action button (FAB) launching `ActivityResultContracts.OpenDocument` for cloud or external OTG storage videos.

---

### Phase 6: Native Compose Navigation & App Engine Flow
...
### Phase 7: App and Video Orientation & Sensor Logic
